		...
		if(mysql_num_rows($res)>0) {
			while($lig=mysql_fetch_object($res)) {
				...
				elseif($lig->destinataire=='pp') {
					$sql="SELECT DISTINCT u.nom,u.prenom,u.email FROM utilisateurs u, j_eleves_professeurs jep, j_eleves_classes jec WHERE jec.id_classe='".$tab_id_classe[$i]."' AND jec.login=jep.login AND jec.id_classe=jep.id_classe AND jep.professeur=u.login AND u.email!='';";
				}
