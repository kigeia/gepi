...
function purge_dossier_photos($type_utilisateurs) {
	...
	// $tab_identifiants : tableau des login ou elenoet pr�sents dans la base
	$tab_identifiants=array(); $pt=0;
	// pour les �l�ves on cherchera parmi les fichiers elenoet.jpg
	...
	// pour les personnels (et pour les �l�ves en multisite) on cherchera parmi les fichiers login.jpg
	...
	// $tab_identifiants_inactifs : tableau des login ou elenoet des comptes inactifs pr�sents dans la base
	...
	if (isset($_POST['cpts_inactifs']) && $_POST['cpts_inactifs']=="oui")
		...
		if ($R_inactifs)
			...
			while ($pt<mysql_num_rows($R_inactifs))
				{
				// dans tous les cas (�l�ves ou personnels) on cherchera parmi les fichiers login.jpg
				$identifiant=mysql_result($R_inactifs,$pt,'login');
				if ($type_utilisateurs=="personnels") $identifiant=md5(strtolower($identifiant));
				$tab_identifiants_inactifs[]=$identifiant;
				// dans le cas des �l�ves on cherchera �galement parmi les fichiers elenoet.jpg
				if ($type_utilisateurs=="eleves")
					{
...
// d�place tous les fichiers du dossier $chemin dans le dossier $destination
// ! si deux fichiers de m�me nom se trouvent dans $chemin un seul sera d�plac�
	$erreurs="";
...
// $type_a_traiter :  : '�l�ve' ou 'personnel'
{
	...
	if($test_folder && !file_exists($folder)) {
		...
		if($nb_photos>0) {$msg_nb_trts.=$nb_photos." photo(s) ".$type_a_traiter."(s) transf�r�e(s).<br/>\n";}
		closedir($dossier);
	}
}
...
// Affichage du personnel sans photo
	if(isset ($_POST['voirPerso']) && $_POST['voirPerso']=="yes"){
		...
		$msg .= "Erreur lors de la s�lection de professeur(s) sans photo";
		}else{
		...
		if (cree_zip_archive("photos")==TRUE)
			...
			if (isset($GLOBALS['multisite']) AND $GLOBALS['multisite']=='y')
				...
				if (!$repertoire_photos=$_COOKIE['RNE'])
					...
					$msg_multisite.="Multisite : erreur lors de la r�cup�ration du dossier photos de l'�tablissement.<br/>";
				...
				if ($nb_photos_supp>0)
					if ($nb_photos_supp>1) $msg=$nb_photos_supp." photos ont �t� suprim�es.<br/>";
						else $msg="Une photo a �t� suprim�e.<br/>";
					else $msg="Aucune photo n'a �t� supprim�e.<br/>";
				$post_reussi=TRUE;
				if ($nb_erreurs>0)
					{
					if ($nb_erreurs>1) $msg.=$nb_erreurs." photos n'ont pu �tre supprim�es.<br/>";
						else $msg.="Une photo n'a pu �tre supprim�e.<br/>";
					$post_reussi=FALSE;
					}
				} else $msg=$msg_multisite.$msg;
			}
		else $msg.="Erreur lors de la cr�ation de la sauvegarde.<br/>";
		}

// Liste des donn�es �l�ve
if (isset($_GET['liste_eleves']) and ($_GET['liste_eleves']=='oui'))  {
	...
	echo "\"classe\",\"nom\",\"pr�nom\",\"pr�nom nom\",\"login\",\"elenoet\"\n";
...
if (isset($_POST['action']) and ($_POST['action']=='upload_photos_eleves'))  {
	...
	if ($sav_file) {
		...
		if (is_file($dir_temp) && !@unlink($dir_temp)) $msg.="Impossible de supprimer ".$dir_temp.".<br/>\n";
				...
				if (!@mkdir($dir_temp,0700,true)) $msg.="Impossible de cr�er ".$dir_temp."..<br/>\n";
		if ($msg=="") {
			// astuce : pour rester compatible avec le script de restauration
			// on cr�e l'arborescence /photos/eleves
			$dir_temp_photos_eleves=$dir_temp."/photos/eleves";
			if (!file_exists($dir_temp_photos_eleves)) 
				if (!@mkdir($dir_temp_photos_eleves,0700,true)) $msg.="Impossible de cr�er ".$dir_temp_photos_eleves.".<br/>\n"; ;
			if ($msg=="") {
				...
				if ($reponse!="ok") {
					...
					// quelque soit la structure du fichier .zip on d�place les photos dans $dir_temp_photos_eleves
					aplanir_tree($dir_temp_photos_eleves,$dir_temp_photos_eleves);

					// on renomme �ventuellement les photos
					...
					if ($msg_multisite=="")
						...
						if ($repertoire_photos!="") $repertoire_photos.="/";
						$repertoire_photos="../photos/".$repertoire_photos;
						$msg_nb_trts=""; // nb de fichiers trait�s
						// copie des fichiers vers /photos
						copie_temp_vers_photos($nb_photos_eleves,'eleves','�l�ve');
						if ($msg_nb_trts=="") $msg_nb_trts="Aucune photo n'a �t� transf�r�e.<br/>\n";
						if ($msg==""){
							$msg= $msg_nb_trts;
							$post_reussi=TRUE;
	...
	// quoiqu'il se soit pass� on supprime le dossier ../temp/trombinoscopes
			...
			if ($reponse!="ok") {
				...
				if ($reponse!="ok") {
					...
					if ($msg_multisite=="")
						...
						// copie des fichiers vers /photos
						$msg_nb_trts=""; // nb de fichiers trait�s
						$avertissement=""; // si l'arborescence est incompl�te
						//El�ves
						copie_temp_vers_photos($nb_photos_eleves,'eleves','�l�ve',true);
						//Personnels
